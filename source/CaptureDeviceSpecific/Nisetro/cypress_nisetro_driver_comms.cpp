...
static void cypress_driver_function(bool* usb_thread_run, CypressDeviceCaptureReceivedData* cypress_device_capture_recv_data, cyni_device_device_handlers* handlers) {
	...
	int dummy = 0;
	cypress_device_capture_recv_data[i].cb_data.is_transfer_data_ready_mutex->general_timed_lock(&dummy);
}

static bool cypress_driver_bulk_async_start(HANDLE handle, uint8_t* buffer, size_t size, DWORD* num_read, uint8_t ep_num, OVERLAPPED* overlap) {
...
int cypress_driver_bulk_in(cyni_device_device_handlers* handlers, const cyni_device_usb_device* usb_device_desc, uint8_t* buf, int length, int* transferred) {
	...
	if(!cypress_driver_bulk_async_start(handlers->read_handle, buf, length, &inner_transfered, usb_device_desc->ep_bulk_in, NULL))
		result = -1;
...
int cypress_driver_ctrl_bulk_in(cyni_device_device_handlers* handlers, const cyni_device_usb_device* usb_device_desc, uint8_t* buf, int length, int* transferred) {
	...
	#ifdef _WIN32
	DWORD inner_transfered = 0;
	if(!cypress_driver_bulk_async_start(handlers->read_handle, buf, length, &inner_transfered, usb_device_desc->ep_ctrl_bulk_in, NULL))
		result = -1;
	*transferred = inner_transfered;
	#endif
	return result;
...
int cypress_driver_ctrl_bulk_out(cyni_device_device_handlers* handlers, const cyni_device_usb_device* usb_device_desc, uint8_t* buf, int length, int* transferred) {
	...
	#ifdef _WIN32
	DWORD inner_transfered = 0;
	if(!cypress_driver_bulk_async_start(handlers->write_handle, buf, length, &inner_transfered, usb_device_desc->ep_ctrl_bulk_out, NULL))
		result = -1;
	*transferred = inner_transfered;
	#endif
	return result;
...
int cypress_driver_async_in_start(cyni_device_device_handlers* handlers, const cyni_device_usb_device* usb_device_desc, uint8_t* buf, int length, cyni_async_callback_data* cb_data) {
	...
	int retval = cypress_driver_bulk_async_start(cb_data->handle, cb_data->buffer, cb_data->requested_length, NULL, usb_device_desc->ep_bulk_in, overlap_data);
	cb_data->transfer_data_access.unlock();
	if(!retval) {
...
void cypress_driver_close_thread(std::thread* thread_ptr, bool* usb_thread_run, CypressDeviceCaptureReceivedData* cypress_device_capture_recv_data) {
	...
	*usb_thread_run = false;
	cypress_device_capture_recv_data[0].cb_data.is_transfer_data_ready_mutex->specific_unlock(0);
	thread_ptr->join();
	for (int i = 0; i < NUM_CAPTURE_RECEIVED_DATA_BUFFERS; i++) {
...
void cypress_driver_cancel_callback(cyni_device_device_handlers* handlers, const cyni_device_usb_device* usb_device_desc, cyni_async_callback_data* cb_data) {
	...
	DeviceIoControl(handlers->read_handle, IOCTL_ADAPT_ABORT_PIPE, &pipe, sizeof(uint8_t), NULL, 0, &transferred, NULL);
	#endif
}
...
void cypress_driver_wait_async_transfers(CypressDeviceCaptureReceivedData* cypress_device_capture_recv_data) {
	#ifdef _WIN32
...
bool cypress_driver_process_async_transfers(CypressDeviceCaptureReceivedData* cypress_device_capture_recv_data, cyni_device_device_handlers* handlers) {
	bool found = false;
	#ifdef _WIN32
	for (int i = 0; i < NUM_CAPTURE_RECEIVED_DATA_BUFFERS; i++) {
		cyni_async_callback_data* cb_data = (cyni_async_callback_data*)&cypress_device_capture_recv_data[i].cb_data;
		...
		if(cypress_device_capture_recv_data[i].in_use && possible_result) {
			...
			if(error != ERROR_IO_INCOMPLETE) {
				...
				cypress_device_capture_recv_data[i].cb_data.is_transfer_data_ready_mutex->specific_unlock(i);
				found = true;
			}
		}
	}
