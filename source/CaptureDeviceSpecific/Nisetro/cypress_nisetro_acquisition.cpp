...
void cypress_output_to_thread(CaptureData* capture_data, CaptureReceived* capture_buf, CaptureScreensType curr_capture_type, std::chrono::time_point<std::chrono::high_resolution_clock>* clock_start, size_t read_size, size_t* scheduled_special_read) {
...
int cypress_device_read_frame_and_output(CaptureData* capture_data, CaptureReceived* capture_buf, CaptureScreensType curr_capture_type, std::chrono::time_point<std::chrono::high_resolution_clock> &clock_start, size_t* scheduled_special_read) {
	const cyni_device_usb_device* usb_device_info = (const cyni_device_usb_device*)capture_data->status.device.descriptor;
	...
	cypress_output_to_thread(capture_data, capture_buf, curr_capture_type, &clock_start, read_size, scheduled_special_read);
	return ret;
}
...
static void cypress_device_read_frame_cb(void* user_data, int transfer_length, int transfer_status) {
	...
	*cypress_device_capture_recv_data->last_index = cypress_device_capture_recv_data->index;
	...
	if((*cypress_device_capture_recv_data->scheduled_special_read) || (*cypress_device_capture_recv_data->is_active_special_read))
		return end_cypress_device_read_frame_cb(cypress_device_capture_recv_data);

	cypress_output_to_thread(cypress_device_capture_recv_data->capture_data, &cypress_device_capture_recv_data->buffer, cypress_device_capture_recv_data->curr_capture_type, cypress_device_capture_recv_data->clock_start, transfer_length, cypress_device_capture_recv_data->scheduled_special_read);
...
static bool process_async_transfers(CypressDeviceCaptureReceivedData* cypress_device_capture_recv_data, cyni_device_device_handlers* handlers) {
	if(handlers->usb_handle)
		return true;
	return cypress_driver_process_async_transfers(cypress_device_capture_recv_data, handlers);
}

static void wait_async_transfers(CypressDeviceCaptureReceivedData* cypress_device_capture_recv_data, cyni_device_device_handlers* handlers) {
	if(handlers->usb_handle)
		return;
...
void wait_all_cypress_device_buffers_free(CaptureData* capture_data, CypressDeviceCaptureReceivedData* cypress_device_capture_recv_data) {
	if(cypress_device_capture_recv_data == NULL)
		return;
	cyni_device_device_handlers* handlers = (cyni_device_device_handlers*)capture_data->handle;
	process_async_transfers(cypress_device_capture_recv_data, handlers);
	...
	for(int i = 0; i < NUM_CAPTURE_RECEIVED_DATA_BUFFERS; i++)
		while(cypress_device_capture_recv_data[i].in_use) {
			...
			wait_async_transfers(cypress_device_capture_recv_data, handlers);
			bool in_processing = process_async_transfers(cypress_device_capture_recv_data, handlers);
			if(in_processing)
